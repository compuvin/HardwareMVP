...
PSTbl = "lansweeper"
PSFN = "Lansweeper" 'Friendly Name for module
PSRunInt = 1 'Module run interval (in days)
PSML = 0 'Is this the master list?
CSVPath = strCurDir & "\source\Asset List - Windows Devices.csv"
...
If filesys.FileExists(CSVPath) then
	...
	str = "CREATE TABLE IF NOT EXISTS " & PSSchema & "." & PSTbl & " (ID INT PRIMARY KEY AUTO_INCREMENT, Name text, IPAddress text, Manufacturer text, Model text, SerialNumber text, FirstDiscovered date DEFAULT NULL, LastDiscovered date DEFAULT NULL);"
	adoconn.Execute(str)
...
Function IngestCSV()
	Dim CurrPC, IPAddress, Manufacturer, Model, SerialNumber

	'PCs - Whats new/old/changed
	AllHW = right(AllHW,len(AllHW)-106)
	AllHW = replace(AllHW,";",",") 'Lansweeper's CSV is really semicolen separated
	do while len(AllHW) > 10
		'Get PC name
		if left(AllHW,1)="""" then
			CurrPC = mid(AllHW,2,instr(1,AllHW,""",",1)-2)
		...
		'Get Model
		if left(AllHW,1)="""" then
			Model = mid(AllHW,2,instr(1,AllHW,""",",1)-2)
			AllHW = right(AllHW,len(AllHW)-instr(1,AllHW,""",",1)-1)
		else
			Model = mid(AllHW,1,instr(1,AllHW,",",1)-1)
			AllHW = right(AllHW,len(AllHW)-instr(1,AllHW,",",1))
		end if
		'msgbox Model
		'Get Serial Number
		...
		'msgbox SerialNumber
		'Get Lastseen Date (not used)
		if left(AllHW,1)="""" then
			AllHW = right(AllHW,len(AllHW)-instr(1,AllHW,""",",1)-1)
		else
			AllHW = right(AllHW,len(AllHW)-instr(1,AllHW,",",1)-1)
		end if
		'Get Lasttried Date (not used)
		if left(AllHW,1)="""" then
		...
		'msgbox CurrPC & vbCrlf & IPAddress & vbCrlf & Manufacturer & vbCrlf & Model & vbCrlf & Serialnumber
		...
		if len(CurrPC) > 0 then
			...
			if not rs.eof then
				...
				if instr(1,outputl,"<p><b>Hardware Added:</b></p>",1) = 0 then
					...
					outputl = outputl & "  <th>Model</th>" & vbcrlf
				...
				outputl = outputl & "  <td>" & Model & "</td>" & vbcrlf
				...
				str = "INSERT INTO "  & PSSchema & "." & PSTbl & "(Name, IPAddress, Manufacturer, Model, SerialNumber, FirstDiscovered, LastDiscovered) values('" & CurrPC & "','" & IPAddress & "','" & Manufacturer & "','" & Model & "','" & SerialNumber & "','" & format(date(), "YYYY-MM-DD")  & "','" & format(date(), "YYYY-MM-DD") & "');"
				adoconn.Execute(str)
...
Function RemoveDevices()
	if PSML = 0 then 'Only run if this is not the master list. Master lists are handled by the main script
		...
		do while not rs.eof
			...
			outputl = outputl & "  <td>" & rs("Manufacturer") & "</td>" & vbcrlf
			outputl = outputl & "  <td>" & rs("Model") & "</td>" & vbcrlf
			outputl = outputl & "  <td>" & rs("SerialNumber") & "</td>" & vbcrlf
			if PConML = 0 then
